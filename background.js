...
chrome.runtime.onInstalled.addListener(function(details) {
  ...
  if (details.reason === 'update') {
    console.log('🎯 Unhook YouTube: Extension updated from version', details.previousVersion);
          ...
          chrome.tabs.query({ url: 'https://www.youtube.com/*' }, (tabs) => {
            tabs.forEach(tab => {
              chrome.tabs.sendMessage(tab.id, { 
                ...
                console.log('🎯 Unhook YouTube: Could not send message to tab', tab.id, err);
              });
            });
          });
...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
  ...
  if (changeInfo.status === 'complete' && 
    ...
    console.log('🎯 Unhook YouTube: YouTube tab updated, ensuring settings are applied');
    ...
    setTimeout(() => {
      chrome.tabs.sendMessage(tabId, { 
        ...
        console.log('🎯 Unhook YouTube: Content script not ready yet for tab', tabId);
      });
    }, 1000);
  }
});
